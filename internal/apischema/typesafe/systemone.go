...
// SystemOneError is the minimal JSON body the gateway synthesizes when the
// upstream returns a non-JSON error. Native JSON errors are passed through
// untouched.
type SystemOneError struct {
	Error SystemOneErrorDetail `json:"error"`
}

// SystemOneErrorDetail carries the error message.
