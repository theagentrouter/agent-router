...
func TestTypeSafeToTypeSafeTranslatorSystemOne_ResponseError(t *testing.T) {
	...
	t.Run("non_json_error", func(t *testing.T) {
		respHeaders := map[string]string{statusHeaderName: "529", contentTypeHeaderName: "text/plain"}
		...
		require.Equal(t, "Overloaded", typesafeErr.Error.Message)
	})

	t.Run("json_error_passthrough", func(t *testing.T) {
		respHeaders := map[string]string{statusHeaderName: "422", contentTypeHeaderName: jsonContentType}
		headerMutation, bodyMutation, err := translator.ResponseError(respHeaders, strings.NewReader(`{"detail":"criteria must have at least 2 levels"}`))
		require.NoError(t, err)
		require.Nil(t, headerMutation)
		require.Nil(t, bodyMutation)
	})
